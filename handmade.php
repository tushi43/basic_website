...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM hproducts WHERE code='" . $_GET["code"] . "'");
			...
			$a=$_POST["quantity"];
			$product_Code = $db_handle->runQuery("UPDATE hproducts SET quant= quant - $a WHERE code='" . $_GET["code"] . "'");
		}
	break;
	case "remove":
		if(!empty($_SESSION["cart_item"])) {
	...
	case "empty":
		unset($_SESSION["cart_item"]);
		$product_Code = $db_handle->runQuery("UPDATE hproducts SET quant= 15");
	 break;	
}
}
?>
...
<div class="txt-heading">Shopping Cart<a id="btnEmpty" href="handmade.php?action=empty">Empty Cart</a></div>
<?php
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		?>
				...
				<td style="text-align:center;border-bottom:#F0F0F0 1px solid;"><a href="handmade.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction">Remove Item</a></td>
				</tr>
				<?php
...
<div id="product-grid">
	...
	<?php
	$product_array = $db_handle->runQuery("SELECT * FROM hproducts ORDER BY id ASC");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item">
			<form method="post" action="handmade.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
